...
import poly.persistance.mongo.comm.AbstractMongoDBCommon;
import static poly.util.CmmUtil.nvl;;

@Component("MelonMapperUsingDTO")
public class MelonMapperUsingDTO extends AbstractMongoDBCommon implements IMelonMapperUsingDTO {
	...
	public int insertSong(List<MelonDTO> pList, String colNm) throws Exception {
		...
		while (it.hasNext()) {
			...
			String collectTime = nvl(rDTO.getCollect_time());
			String song = nvl(rDTO.getSong());
			String singer = nvl(rDTO.getSinger());
	...
	public List<MelonDTO> getSongList(String colNm) throws Exception {
		...
		// 조회 결과를 전달히기 위한 객체 생성하기
		List<MelonDTO> rList = new LinkedList<MelonDTO>();
		
		MongoCollection<Document> col = mongodb.getCollection(colNm);
		
		// 조회결과 중 출력할 컬럼들 (SQL의 SELECT절과 FROM절 가준데 컬럼들과 유사함)
		...
		while(cursor.hasNext()) {
			...
			// 조회 잘되나 출력해봄
			String song = nvl(doc.getString("song"));
			String singer = nvl(doc.getString("singer"));
