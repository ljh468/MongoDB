...
public class MelonMapper implements IMelonMapper {
	...
	public boolean createCollection(String colnm) throws Exception {
		...
		// 기존에 등록된 컬렉션 이름이 존재하는지 체크하고, 존재하면 기존컬렉션 삭제함
	...
	public List<MelonDTO> getRank(String colNm) throws Exception {
		...
		// 컬렉션으로부터 전체 데이터 가져온 걸을 List형태로 저장하기 위한 변수 선언
	...
	public List<MelonSingerDTO> getRankForSinger(String colNm) throws Exception {
		...
		List<DBObject> pipeline = Arrays.asList(
				// SQL 의 Group by와 같은 역할을 수행하는 MongoDB group 함수 호출
                new BasicDBObject()
                        .append("$group", new BasicDBObject()
                                .append("_id", new BasicDBObject()
                                        .append("singer", "$singer") // 그룹으로 묶을 피드
                                )
                                // 그룹으로 묶인 함수를 통해 계산될 내용 (레코드 수 세기)
                                .append("COUNT(singer)", new BasicDBObject()
                                        .append("$sum", 1) // count는 레코드 별로 1씩 더하는 것과 동일함
                                )
                        ), 
                // project는 결과 보여줄내용, SQL의 select과 form절 사이 내용으로 이해하면 편함
                new BasicDBObject()
                        .append("$project", new BasicDBObject()
                                .append("singer", "$_id.singer")
                                .append("song_cnt", "$COUNT(singer)")
                                .append("_id", 0)
                        ), 
                // SQL의 order by와 같은 역할을 수행하는 mongodb sort 함수 호출
                // 1차 정렬은 song_cnt인 노래수가 큰 순서, 2차정렬은 가수이름의 가나다 순서
                new BasicDBObject()
	...
	public List<MelonRankDTO> getCompareRank(String curColNm, String preColNm) throws Exception {
		...
		List<DBObject> pipeline = Arrays.asList(
				...
				// 조회되는 데이터 표시를 위한 항목
                 ...
                 // MongoDB는 조인이 없기 때문에 LookUp을 통해 유사하게 구현 가능함
                 new BasicDBObject()
		...
		// 가상메모리 사용
         ...
         // 컬렉션으로부터 전체 데이터 가져온걸을 List 형태로 저장하기 위한 변수 선언
         List<MelonRankDTO> rList = new ArrayList<MelonRankDTO>();
         
         log.info("cursor null? : " + (cursor == null));
         //
         ...
         while(cursor.hasNext()) {
        	 ...
        	 // mongoDB project에서는 필드별 사칙연산을 제공하지 않기에 자바에서 처리
        	 int change = Math.abs(current_rank - pre_rank); // 랭킹변화(절대값 변환)
